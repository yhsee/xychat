...
const
  ...
  procedure EncryStream(var TmpStream: TMemoryStream;const Key: String);
  procedure DecryStream(var TmpStream: TMemoryStream;const Key: String);
...
begin
  ...
  SetLength(sTmpString,TmpStream.size);
  TmpStream.Read(sTmpString[1],TmpStream.Size);
  sTmpStr:=EncryStr(UTF8Encode(sTmpString),Key);
  TmpStream.Size:=Length(sTmpStr);
  TmpStream.Position:=0;
  TmpStream.Write(sTmpStr[1],TmpStream.Size);
  TmpStream.Position:=0;
end;
...
begin
  TmpStream.Position:=0;
  SetLength(sTmpStr,TmpStream.size);
  TmpStream.Read(sTmpStr[1],TmpStream.Size);
  sTmpString:=UTF8Decode(DecryStr(sTmpStr,Key));
  TmpStream.Size:=Length(sTmpString);
  TmpStream.Position:=0;
  TmpStream.Write(sTmpString[1],TmpStream.Size);
  TmpStream.Position:=0;
end;
