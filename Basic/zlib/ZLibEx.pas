...
{** Variant routines *********************************************************}
procedure ZFastCompressOleVariant(var TmpVar:OleVariant;const key:String);
procedure ZFastDecompressOleVariant(var TmpVar:OleVariant;const key:String);
{** string routines *********************************************************}
...
{** Variant routines *********************************************************}
procedure VariantToStream(const TmpVar:OleVariant;TmpStream:TMemoryStream);
var
  P:Pointer;
begin
  TmpStream.Position:=0;
  TmpStream.Size:=VarArrayHighBound(TmpVar, 1) + 1;
  P:=VarArrayLock(TmpVar);
  TmpStream.Write(P^,TmpStream.Size);
  VarArrayUnlock(TmpVar);
  TmpStream.Position:=0;
end;

procedure StreamToVariant(TmpStream:TMemoryStream;var TmpVar:OleVariant);
var
  P:Pointer;
begin
  TmpVar := VarArrayCreate([0,TmpStream.Size-1],varByte);
  P:=VarArrayLock(TmpVar);
  TmpStream.Position:=0;
  TmpStream.Read(P^,TmpStream.Size);
  VarArrayUnlock(TmpVar);
end;
//------------------------------------------------------------------------------
procedure ZFastCompressOleVariant(var TmpVar:OleVariant;const key:String);
...
begin
  ...
  VariantToStream(TmpVar,TmpInStream);
  ZCompressStream(TmpInStream,TmpOutStream);
  EncryStream(TmpOutStream,key);
  StreamToVariant(TmpOutStream,TmpVar);
  finally
  freeandnil(TmpOutStream);
  freeandnil(TmpInStream);
  end;
end;
...
begin
  ...
  TmpOutStream:=TMemoryStream.Create;
  VariantToStream(TmpVar,TmpInStream);
  DecryStream(TmpInStream,key);
  ZDeCompressStream(TmpInStream,TmpOutStream);
  StreamToVariant(TmpOutStream,TmpVar);
  finally
  freeandnil(TmpOutStream);
  freeandnil(TmpInStream);
